...
var OptionsPage = new function OptionsPage() {
	...
	/**
	 * @private
	 * Sends a 'settings' event
	 */
	function sendChangedEvent(key, oldValue, newValue) {
		if (typeof CustomEvent === 'undefined')
			return;

		var e = new CustomEvent('settings', {
			detail: {
				key: key,
				oldValue: oldValue,
				newValue: newValue,
	...
	function elementChanged(e) {
		...
		sendChangedEvent(name, oldval, value);
	}
	...
	function addResetButton(button, element) {
		button.addEventListener('click', function() {
			var oldval = storage.get(element.name);
			storage.reset(element.name);
			updateElement(element);
			sendChangedEvent(element.name, oldval, storage.get(element.name));
		}, false);
	}
